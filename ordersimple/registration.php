...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $statement = $mysql->prepare("INSERT INTO users (first_name, phone_number, password, email, role) VALUES (?, ?, ?, ?, ?)");
        if (!$statement) {
            die("Ошибка подготовки запроса: " . $mysql->error);
        }

        $statement->bind_param("sssss", $first_name, $phone_number, $password, $email, $role);
